...
EXE-файл работает как на 32-разрядных, так и на 64-разрядных системах Windows. Его можно запускать , например, такой командой:
```CMD
start "snoop" "C:\UTIL\AHK\snoop.exe"
```
Для запуска при старте системы нужно разместить ссылку на исполняемый CMD или EXE-файл в папке `"%userprofile%\AppData\Roaming\Microsoft\Windows\Start Menu\Programs\Startup"`.
